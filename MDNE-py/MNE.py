...
# <hzhangal@cse.ust.hk>
# reference:
# https://github.com/HKUST-KnowComp/MNE/blob/master/MNE.py

from __future__ import division  # py3 "true division"

import logging  #to provide a standard error logging mechanism in Python
import sys
import os
import heapq
...
from numpy import exp, log, dot, zeros, outer, random, dtype, float32 as REAL,\
    double, uint32, seterr, array, uint8, vstack, hstack, fromstring, sqrt, newaxis,\
...
def get_G_from_edges(edges):
    edge_dict = dict()
    for edge in edges:
        edge_key = str(edge[0]) + '_' + str(edge[1])  #为了str
        if edge_key not in edge_dict:
            edge_dict[edge_key] = int(edge[2])
        else:
            edge_dict[edge_key] += int(edge[2])
            print('Catherine warning: not repeat edge')
    tmp_G = nx.DiGraph()
    for edge_key in edge_dict:
        weight = edge_dict[edge_key]
        tmp_G.add_edge(edge_key.split('_')[0], edge_key.split('_')[1])  #add_edge可能要求输入string
        tmp_G[edge_key.split('_')[0]][edge_key.split('_')[1]]['weight'] = weight
    return tmp_G    #得到['1'，'13'，1; '1','16',1; ......]
...
def load_network_data(f_name):
    ...
    with open(f_name, 'r') as f:
        for line in f:
            words = line.split(',')[:-1]    #line[:-1]去除列表的最后一个元素
            if words[0] not in edge_data_by_type:
                edge_data_by_type[words[0]] = list()
            edge_data_by_type[words[0]].append((words[1], words[2],words[3]))   #加入次数属性
            all_edges.append((words[1], words[2],words[3]))
            all_nodes.append(words[1])
            all_nodes.append(words[2])
    all_nodes = list(set(all_nodes))        #转换成集合，自动排序+去重
    # create common layer.
    all_edges = list(set(all_edges))
    edge_data_by_type['Base'] = all_edges   #key值现在有1，2，3，Base
...
def train_embedding(current_embedding, walks, layer_id, iter=10, info_size=10, base_weight=1):
    ...
    # 是类初始化形成的句柄赋值给了new_model
    new_model = MNE(training_data, size=200, window=1, min_count=0, sg=1, workers=4, iter=iter, small_size=info_size,
                    directed=True, initial_embedding=initial_embedding, base_weight=base_weight)
...
def train_model(network_data, only_base = False, network_data2=None):
    small_size = 32     # addition_embedding 的尺寸
    base_network = network_data['Base']    #无重复的边的list，也就是合起来的总公交出行边
    base_G = biased_Random_walk.RWGraph(get_G_from_edges(base_network), 'directed', 1, 1) #相当于biased_Random_walk里的类来初始化了一下
    transfer_network = network_data2['Base']  # 总的公交出行换乘边
    global transfer_G
    transfer_G = biased_Random_walk.RWGraph(get_G_from_edges(transfer_network), 'directed', 1, 1)  #公交出行换乘图
    print('finish building the graph')
    base_G.preprocess_transition_probs()
    transfer_G.preprocess_transition_probs()
    base_walks = base_G.simulate_walks(20, 10)
    base_embedding, _, _, index2word = train_embedding(None, base_walks, 'Base', 50, small_size, 1)
    final_model = dict()
    ...
    if (only_base):
        return final_model   # wayne 因为聚类暂时用不到后面的且base向量后面也不更新了，为了输出速度，执行到这里就可以输出吧
    for layer_id in network_data:
        if layer_id == 'Base':
            continue
        print('We are training model for layer:', layer_id)
        if layer_id not in final_model['addition']:
            final_model['addition'][layer_id] = zeros((len(final_model['index2word']), small_size), dtype=REAL)   #initialization
        tmp_data = network_data[layer_id]
        # start to do the random walk on a layer
        layer_G = biased_Random_walk.RWGraph(get_G_from_edges(tmp_data), 'directed', 1, 1)
        layer_G.preprocess_transition_probs()
        layer_walks = layer_G.simulate_walks(20, 10)
        tmp_base, tmp_tran, tmp_local, tmp_index2word = train_embedding(final_model, layer_walks, layer_id, 10, small_size, 0)
...
def save_model(final_model, save_folder_name):
    ...
    np.savetxt(save_folder_name+'/base.csv', final_model['base'], delimiter=",")
    for layer_id in final_model['addition']:
        ...
        np.savetxt(save_folder_name+'/tran_'+str(layer_id)+'.csv', final_model['tran'][layer_id], delimiter=",")
        np.savetxt(save_folder_name+'/addition_'+str(layer_id)+'.csv', final_model['addition'][layer_id], delimiter=",")

def save_model_base(final_model, save_folder_name):
    with open(save_folder_name+'/'+'index2word.json', 'w') as f:
        json.dump(final_model['index2word'], f)
    np.save(save_folder_name+'/base.npy', final_model['base'])
    np.savetxt(save_folder_name+'/base.csv', final_model['base'], delimiter=",")
...
def train_batch(model, sentences, alpha, limitation, base_weight, work=None):
    ...
    global BATCH_NUM
    global transfer_G
    print('training model for batch:', BATCH_NUM)
    for sentence in sentences:
        word_vocabs = [model.wv.vocab[w] for w in sentence if w in model.wv.vocab and
                        model.wv.vocab[w].sample_int > model.random.rand() * 2**32]
        for pos, word in enumerate(word_vocabs):
            reduced_window = model.random.randint(model.window)  # `b` in the original word2vec code
            reduced_window = 0 # wayne 因为我这里的window本来就打算取1，就不用这个参数了

            # now go over all words from the (reduced) window, predicting each one in turn
            if model.directed:
                start = pos     #好像只取了[j-c,j+c]的后半部分
            else:
                start = max(0, pos - model.window + reduced_window)   #无向图会取到[j-c,j+c]全部
            for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                ...
                if pos2 != pos:
                    ...
                    # wayne 下面是多算一次和换乘站点的关系
                    current_node = model.wv.index2word[word.index]
                    transfer_node = transfer_G.walk_one(current_node)  # wanye find current transfer node
                    # wayne 要满足:1）当前节点有近邻的换乘节点 2）当前节点的换乘节点不是自己 3）这个换乘节点要在建立的词典中
                    if (transfer_node != 'No' and transfer_node != current_node and transfer_node in model.wv.vocab):
                        temp_word2 = model.wv.vocab[transfer_node]
                        train_pair(model, model.wv.index2word[word.index], temp_word2.index, alpha, limitation=limitation, base_weight=base_weight)
...
def train_pair(model, word, context_index, alpha, limitation, base_weight, learn_vectors=True, learn_hidden=True,
    ...
    l1 = model.in_base[context_index] + np.dot(model.in_local[context_index], model.in_tran)  #对应原文的公式(1)
    ...
    l2b = model.syn1neg[word_indices]  # 2d matrix, k+1 x layer1_size  #对应原文的公式(5-8)里e中的:nk
    fb = expit(dot(l1, l2b.T))  # propagate hidden -> output  #对应原文的公式(5-8)中的:e
    gb = (model.neg_labels - fb) * alpha  # vector of error gradients multiplied by the learning rate #对应原文的公式(5-8)中的:学习率*e

    if learn_hidden:
        model.syn1neg[word_indices] += outer(gb, l1)  # learn hidden -> output  #对应原文的公式(6)
    neu1e += dot(gb, l2b)  # save error

    # 震惊：他这里把权重base_weight设置成1或0，那么其实第一遍跑完得到base_embedding后面就不再更新了，同理第一遍也不更新local_embedding
    if learn_vectors:
        model.in_base[context_index] += base_weight*neu1e    #对应原文的公式(5)
        model.in_tran += (1-base_weight)*outer(model.in_local[context_index], neu1e * lock_factor)   #对应原文的公式(8)
        model.in_local[context_index] += (1-base_weight)*dot(neu1e * lock_factor, model.in_tran.T)   #对应原文的公式(7)
...
class MNE(utils.SaveLoad):
    ...
    def __init__(
        ...
        `sample` = threshold for configuring which higher-frequency words are randomly downsampled;
            default is 1e-3, useful range is (0, 1e-5).  因为公交网络不存在频繁词the of等的采样，这里设置成0
        ...
        if sentences is not None:
            if isinstance(sentences, GeneratorType):        #Iterator in python is any python type that can be used with a ‘for in loop’
                raise TypeError("You can't pass a generator as the sentences argument. Try an iterator.")
            self.build_vocab(sentences, trim_rule=trim_rule)   #先建立词典
            self.train(sentences)                              #再训练
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        self.clear_sims()
        return trained_word_count  #返回的是train这个函数
    ...
    def reset_weights(self):
        ...
        self.in_base = empty((len(self.wv.vocab), self.vector_size), dtype=REAL) #initialization
